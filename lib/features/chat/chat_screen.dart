...
class _SacredHeaderAction extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Semantics(
      ...
      label: semanticLabel,
      child: SizedBox(
        // Visual chip stays 40x40 (matches SacredHeader's back chip);
        // OverflowBox grows only the tappable region to the 44px minimum,
        // so the header Row's layout is unaffected.
        width: 40,
        height: 40,
        child: OverflowBox(
          minWidth: 44,
...
/// Empty-state shown before the first message in a session — a warm welcome
/// plus a few tappable suggested questions so the user isn't staring at a
/// blank screen wondering what to ask.
class _ChatEmptyState extends StatelessWidget {
  final ValueChanged<String> onQuestionTap;

  const _ChatEmptyState({required this.onQuestionTap});
